...
### 應用程式概要
我已經根據你提供的 PDF 內容建立了一個 Streamlit 模擬考試程式。
1. **隨機抽取**：程式每次啟動或重新開始時，都會從題庫中隨機挑選 40 題。
2. **模擬環境**：採用單選題介面，並在提交後即時計算得分。
3. **詳細檢討**：提交後你可以展開每一題，查看你的答案、正確答案以及詳細的合理解釋（Rationale），幫助你真正理解內容而非死記硬背。
